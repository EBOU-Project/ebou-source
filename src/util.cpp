...
const char * const BITCOIN_CONF_FILENAME = "ebou.conf";
const char * const BITCOIN_PID_FILENAME = "eboud.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "eboud -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ebou
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ebou
    // Mac: ~/Library/Application Support/ebou
    // Unix: ~/.ebou
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ebou";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ebou";
#else
    // Unix
    return pathRet / ".ebou";
#endif
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty ebou.conf if it does not excist
